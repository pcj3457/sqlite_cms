...
$query_newslist = sprintf("SELECT news.*,newskind.KindTitle,newskind.ParentID,newskind.KindType FROM news left join newskind  on news.KindID=newskind.KindID  where news.KindID=%s ORDER BY news.xuhao DESC",
...
//是否单独页 判断
if ($all_row_newslist['KindType']==2)
{
	$newsid=$all_row_newslist['NewsID'];
	$goto="endpage.php?NewsID=".$newsid;
	 header(sprintf("Location: %s", $goto));
	
	}
if ($all_row_newslist['KindType']==1)
{
	$kindid=$all_row_newslist['KindID'];
	$goto="listpic.php?kindid=".$kindid;
	 header(sprintf("Location: %s", $goto));
...
<div class="mid">
  ...
  <ul><?php  
	  ...
	  do { ?>
    <li><a href="endpage.php?NewsID=<?php echo $row_newslist['NewsID'];  ?>" class="zwn"><?php echo cnSubStr($row_newslist['title'],40); ?></a><?php echo date("Y-m-d",$row_newslist['indate']);  ?></li><?php }
